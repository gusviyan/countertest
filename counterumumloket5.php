...
<html>
<title>COUNTER 5 Customer Care</title>
<meta charset="UTF-8">
...
<nav class="w3-sidebar w3-bar-block w3-white w3-text-grey w3-collapse w3-top w3-center" style="z-index:3;width:300px;font-weight:bold" id="mySidebar"><br>
  <h3 class="w3-padding-10 w3-center"><b>ANTRIAN HOLD</b></h3>
	...
	<?php
		$act = $_GET['act'];
		$hld = mysql_query ("SELECT * FROM tbl_cs WHERE status = 2 and panggil = 0");
...
<div class="w3-main" style="margin-left:300px">
  ...
  <div class="w3-row" style="margin-top:0">
    <div class="w3-half" style="border: 5px solid black; width: 50%; height:750px">
		...
		if ( $act == "call") {
			$cek = mysql_query ("SELECT * FROM tbl_cs WHERE status = 2 AND panggil = 2 AND loket = 5 LIMIT 1");
			$pgl = mysql_num_rows ($cek);
			if ($pgl == 0){
				$cek = mysql_query ("SELECT * FROM tbl_cs WHERE status = 0  AND panggil = 0 LIMIT 1");
				$pgl = mysql_fetch_array ($cek);
				mysql_query("UPDATE tbl_cs SET  panggil = 1, loket = 5 WHERE id = $pgl[0]");
				mysql_query("UPDATE tbl_cs SET  panggil = 0, loket = 0 WHERE status = 2 AND loket = 5");
				if ($pgl != ''){
					echo"
					<div id='rajal' style='display: block;'>	
					<center>
					<h1 style='font-size:1200%; padding-top: 100px'>CS$pgl[0]</h1>
					</center>
					
					<div class='w3-display-bottomleft w3-large' style='padding-left: 100px; padding-bottom: 200px;'>
					
					<form method=POST action='aksi_panggil.php?act=clear&id=$pgl[0]&loket=5'>
					...
					<form method=POST action='aksi_panggil.php?act=hold&id=$pgl[0]&loket=5'>
					...
					<div class='w3-display-bottomleft w3-large' style='padding-bottom: 200px;'>
					<p style='font-size:200%;'><a href='counterumumloket5.php?act=default'><button onclick='document.getElementById('rajal').style.display=block' class='w3-button w3-black'>Back</button></a></p>
					</div>
					";
				}
			} else {
				$cek = mysql_query ("SELECT * FROM tbl_cs WHERE status = 0  AND panggil = 1 LIMIT 1");
				$pgl = mysql_fetch_array ($cek);
				//mysql_query("UPDATE tbl_cs SET  panggil = 1, loket = 5 WHERE id = $pgl[0]");
				// mysql_query("UPDATE tbl_cs SET  panggil = 0, loket = 0 WHERE status = 2 AND loket = 5");
				if ($pgl != ''){
			...
			$cek = mysql_query ("SELECT * FROM tbl_cs WHERE status = 2 and id=$_GET[no] and panggil = 0");
			$pgl = mysql_fetch_array ($cek);
			mysql_query("UPDATE tbl_cs SET  panggil = 0, loket = 0 WHERE status = 2 AND loket = 5");
			mysql_query("UPDATE tbl_cs SET  panggil = 2, loket = 5 WHERE id = $pgl[0]");
		...
		<center>
		<h1 class='w3-display-bottomleft' style='font-size:300%;padding-bottom: 7%;'> Customer Care
