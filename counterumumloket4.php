...
<nav class="w3-sidebar w3-bar-block w3-white w3-text-grey w3-collapse w3-top w3-center" style="z-index:3;width:300px;font-weight:bold" id="mySidebar"><br>
  <h3 class="w3-padding-10 w3-center"><b>ANTRIAN HOLD</b></h3>
	...
	<?php
		$act = $_GET['act'];
		$hld = mysql_query ("SELECT * FROM tbl_bpjs WHERE status = 2 and panggil = 0");
...
<div class="w3-main" style="margin-left:300px">
  ...
  <div class="w3-row" style="margin-top:0">
    <div class="w3-half" style="border: 5px solid black; width: 50%; height:750px">
		...
		if ( $act == "call") {
			$cek = mysql_query ("SELECT * FROM tbl_bpjs WHERE status = 2 AND panggil = 2 AND loket = 4 LIMIT 1");
			$pgl = mysql_num_rows ($cek);
			if ($pgl == 0){
				$cek = mysql_query ("SELECT * FROM tbl_bpjs WHERE status = 0 AND panggil = 0 LIMIT 1");
				$pgl = mysql_fetch_array ($cek);
				mysql_query("UPDATE tbl_bpjs SET  panggil = 1, loket = 4 WHERE id = $pgl[0]");
				mysql_query("UPDATE tbl_bpjs SET  panggil = 0, loket = 0 WHERE status = 2 AND loket = 4");
				if ($pgl != ''){
					echo"
		...
		$cek = mysql_query ("SELECT * FROM tbl_bpjs WHERE status = 2 and id=$_GET[no]");
		$pgl = mysql_fetch_array ($cek);
			mysql_query("UPDATE tbl_bpjs SET  panggil = 0, loket = 0 WHERE status = 2 AND loket = 4");
			mysql_query("UPDATE tbl_bpjs SET  panggil = 0, loket = 0 WHERE status = 0 AND loket = 4");
			mysql_query("UPDATE tbl_bpjs SET  panggil = 2, loket = 4 WHERE id = $pgl[0]");
